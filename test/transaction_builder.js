...
describe('TransactionBuilder', function () {
  ...
  describe('fromTransaction', function () {
    ...
    fixtures.zcash.valid.forEach(function (testData) {
      it('returns TransactionBuilder, with ' + testData.description, function () {
        ...
        const tx = Transaction.fromHex(testData.hex, network)
        const txb = TransactionBuilder.fromTransaction(tx, network)
      ...
      it('throws if transaction builder network is incompatible for ' + testData.description, function () {
        const errorMessage = 'This transaction is incompatible with the transaction builder'
        const tx = Transaction.fromHex(testData.hex, NETWORKS.zcash)
  ...
  const networksToTest = ['bitcoin', 'bitcoingold', 'bitcoincash', 'zcashTest']
  networksToTest.forEach(function (network) {
    describe('addInput for ' + network, function () {
      const testNetwork = NETWORKS[network]
      let txb
      const testKeyPair = new ECPair(BigInteger.ONE, undefined, { network: testNetwork })
      beforeEach(function () {
      ...
      it('accepts a prevTx, index [and sequence number]', function () {
        const prevTx = new Transaction(testNetwork)
  ...
  networksToTest.forEach(function (network) {
    describe('addOutput for ' + network, function () {
      ...
      it('accepts an address string and value', function () {
        const vout = txb.addOutput(testKeyPair.getAddress(), 1000)
        assert.strictEqual(vout, 0)

        const txout = txb.tx.outs[0]
